...
async def _process_voice(tg_user_id: int, file_id: str):
    """Sesli mesaji transkribe et, metin olarak isle."""
    # Once typing goster
    try:
    ...
    # Transkripsiyon bildirimi gonder
    try:
        await telegram_client.send_message(
            tg_user_id,
    ...
    try:
        ...
        logger.info("Voice transcribed for tg=%s: %s", tg_user_id, transcript[:80])
    except Exception:
        logger.exception("Voice transcription failed for tg=%s", tg_user_id)
        try:
            await telegram_client.send_message(
                tg_user_id,
                "Sesli mesajınızı anlayamadım, yazarak tekrar iletir misiniz?",
            )
        except Exception:
            pass
        return
    ...
    # Transkripti kullaniciya goster
    try:
        await telegram_client.send_message(
            tg_user_id,
            f"🎤 Anladım: \"{transcript}\"",
        )
    except Exception:
        pass

    # Normal metin akisina devam et
    await _process_text(tg_user_id, transcript)
